...
<script>
  ...
  function coba(id, status) {
    ...
    $.ajax({
      url: "<?= site_url('crud-backend/user/status/'); ?>"+id,
      type: "post",
      data: {
        id: id, status: status
      },
  ...
  $(document).ready(function() {
    ...
    table = $('#table').DataTable({
      ...
      "serverSide": true,
      "ajax": {
        "url": "<?= site_url('crud-backend/user/ajaxList') ?>",
        "type": "POST"
      },
      "columnDefs": [{
        "targets": [-1],
  ...
  function edit(id) {
    ...
    $.ajax({
      url: "<?= site_url('crud-backend/user/ajax_edit/') ?>" + id,
      type: "POST",
      dataType: "JSON",
      cache: false,
  ...
  function save() {
    ...
    var url;
    if (save_method == 'add') {
      url = "<?= site_url('crud-backend/user/ajax_add') ?>";
    } else
    {
      url = "<?= site_url('crud-backend/user/ajax_update') ?>";
    }
  ...
  function delete_user(id) {
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        $.ajax({
          url: "<?php echo site_url('crud-backend/user/ajax_delete/') ?>"+id,
          type: "POST",
          dataType: "JSON",
          cache: false,
          success: function(data) {
